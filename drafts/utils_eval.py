...
def normalize_answer(s):
    ...
    def remove_punc(text):
        exclude = set(string.punctuation) # all unduplicated punctuations
...
def compute_pos(f:str, t_gold:int, addition:int, t_pred:List[int]): 
    r"""
    ...
    Returns:
        float: the POS score.
    """
    # In the paper, POS score is to solve the problem of the wrong-tag-right-prediction question counted as wrong prediction
    # for p1, p2 two paths in the DOM tree of the html, POS = |p1 and p2|/|p1 or p2|
    h = BeautifulSoup(open(f))
    p_gold, e_gold = set(), h.find(tid=t_gold)
    if e_gold is None: # find the gold prediction's tag content
        if len(t_pred) != 1:
            return 0
        else:
            t = t_pred[0]
