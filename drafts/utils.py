...
def read_wrc_examples_to_qa(input_file, root_dir, is_training, tokenizer, method, simplify=False):
    r"""
    ...
    def html_to_text(h):
        ...
        tag_list = set()
        for element in h.descendants:
            if type(element) == bs4.element.Tag: # In a html doc, traverse the doc's descendants, when it's a tag
                                                 # take the tag's identifier <xx> </xx>
                                                 # and remove all the attributes, like href, class, etc...
                element.attrs = {}
    ...
    def adjust_offset(offset, text):
        # given char_to_word_offsets of a html doc file, and raw text of the html
        text_list = text.split() # all tokens' list
        cnt, adjustment = 0, []
        for t in text_list:
            if not t:
                continue
            if t[0] == '<' and t[-1] == '>': # it's a tag
                adjustment.append(offset.index(cnt)) # store the offset of tag (word-wise offset/index)
            else:
                cnt += 1
        add = 0
        adjustment.append(len(offset))
        for i in range(len(offset)):    # i is the index of char
            while i >= adjustment[add]: # say, the char's offset/index is larger than the tag's offset
                                        # it's invalid as showing the tag is not the correct one to which the char belong
                add += 1
            offset[i] += add            # so, finally the offset is the char-wise offset. say, chars in identical token 
                                        # share same offset, but the offset is counted in char-wise (originally token-wise)
        return offset
    ...
    def calc_num_from_raw_text_list(t_id, l):
        ...
        for i in range(t_id):
            n_char += len(l[i]) + 1
        return n_char # add all length (including whitespace) before the t-id, to get the offset of start 
    ...
    for entry in input_data:
        ...
        for website in entry["websites"]:
            ...
            raw_text_list, tag_num = html_to_text_list(html_code) # raw_text_list is a list of all contents of 
                                                                  # html doc, removing all tags
            page_text = ' '.join(raw_text_list) # concatenate all to whitespace-separated text (of a page)
            doc_tokens = []
            char_to_word_offset = []
            ...
            # char_to_word_offset stores such a relation on the page_text:
            # all chars (including whitespace) belong to which char?
            # example: "hello my" -> [0,0,0,0,0,0,1,1]
            ...
            if simplify:
                ...
                for (i, token) in enumerate(doc_tokens):
                    ...
                    for sub_token in sub_tokens:
                        tok_to_orig_index.append(i)      # the subtoken belong to which original token?
                        all_doc_tokens.append(sub_token) # all the sub tokens of doc
                ...
                for qa in website["qas"]:
                    ...
                    if is_training:
                        ...
                        if answer["element_id"] == -1: # maybe refer to yes/no answer, and in this case answer starts at -2
